// CO_Code_05 - Coordinator Arduino Code Based on CO_Attributes_01.txt
//  Incorporates serial CMD:
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (cmd.equalsIgnoreCase("CMD:SHUTDOWN")) {
      shutdown(SHUTDOWN_USER_COMMAND);
    }
    else if (cmd.equalsIgnoreCase("CMD:STATUS")) {
      Serial.print("Status - Shutdown State: ");
      Serial.println(shutdownState ? "YES" : "NO");
    }
    else {
...
void shutdown(const char* reason) {
  ...
  // Notify Watchdog with clear message
